...
/// A block assembler for the Kasplex network that implements the `BlockAssembler` trait.
#[derive(Clone, Debug)]
pub struct KasplexBlockAssembler {
    block_assembler: EthBlockAssembler<KasplexChainSpec>,
}

impl KasplexBlockAssembler {
    /// Creates a new instance of the [`KasplexBlockAssembler`] with the given chain specification.
    pub fn new(chain_spec: Arc<KasplexChainSpec>) -> Self {
        Self { block_assembler: EthBlockAssembler::new(chain_spec) }
    }

    /// Returns a reference to the chain specification.
    pub fn chain_spec(&self) -> Arc<KasplexChainSpec> {
        self.block_assembler.chain_spec.clone()
    }
}

impl<F> BlockAssembler<F> for KasplexBlockAssembler
where
    F: for<'a> BlockExecutorFactory<
            ExecutionCtx<'a> = KasplexBlockExecutionCtx<'a>,
        ...
        >,
{
    ...
    /// Builds a Kasplex network block.
    fn assemble_block(
        &self,
