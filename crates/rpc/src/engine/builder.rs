...
use crate::engine::api::KasplexEngineApi;

/// Builder for basic [`EngineApi`] implementation.
///
/// This provides a basic default implementation for Kasplex engine API via
/// [`KasplexEngineTypes`] and uses the general purpose [`EngineApi`] implementation as the builder's
/// output.
#[derive(Debug, Default)]
pub struct KasplexEngineApiBuilder<PVB> {
    payload_validator_builder: PVB,
}

impl<PVB> KasplexEngineApiBuilder<PVB> {
...
impl<N, PVB> EngineApiBuilder<N> for KasplexEngineApiBuilder<PVB>
where
    ...
    PVB::Validator: EngineApiValidator<<N::Types as NodeTypes>::Payload>,
{
    ...
    type EngineApi = KasplexEngineApi<
        N::Provider,
