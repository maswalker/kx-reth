...
#[derive(Debug)]
pub struct KasplexNodeCommand<C: ChainSpecParser, Ext: clap::Args + fmt::Debug = NoArgs>(
    pub Box<NodeCommand<C, Ext>>,
);

impl<C: ChainSpecParser> KasplexNodeCommand<C> {
...
impl<C: ChainSpecParser, Ext: clap::Args + fmt::Debug> KasplexNodeCommand<C, Ext> {
...
impl<C, Ext> KasplexNodeCommand<C, Ext>
where
    C: ChainSpecParser,
    ...
    Ext: clap::Args + fmt::Debug,
{
    ...
    where
        L: Launcher<C, Ext>,
    {
        tracing::info!(target: "reth::kasplex::cli", version = ?version::version_metadata().short_version, "Starting kasplex-reth");
        ...
        tracing::info!(target: "reth::kasplex::cli", path = ?db_path, "Opening database");
        let database = Arc::new(
            init_db_for::<PathBuf, reth_db::Tables>(db_path.clone(), self.0.db.database_args())?
                .with_metrics(),
        );
