...
use crate::{evm::KasplexEvm, handler::KasplexEvmHandler};

// Trait that allows to replay and transact the transaction, we
// use [`KasplexEvmHandler`] to handle the transactions execution, besides
// that, we use the same implementation as `RevmEvm`.
impl<CTX, INSP, P> ExecuteEvm for KasplexEvm<CTX, INSP, P>
where
    ...
    P: PrecompileProvider<CTX, Output = InterpreterResult>,
{
    ...
    fn transact_one(&mut self, tx: Self::Tx) -> Result<Self::ExecutionResult, Self::Error> {
        ...
        KasplexEvmHandler::<_, _, EthFrame>::new().run(self)
    }
    ...
    /// the execution with [KasplexEvmHandler] and return the result.
    fn replay(
        &mut self,
    ) -> Result<ExecResultAndState<Self::ExecutionResult, Self::State>, Self::Error> {
        KasplexEvmHandler::<_, _, EthFrame>::new()
            .run(self)
...
impl<CTX, INSP, P> ExecuteCommitEvm for KasplexEvm<CTX, INSP, P>
where
...
impl<CTX, INSP, P> InspectEvm for KasplexEvm<CTX, INSP, P>
where
    ...
    P: PrecompileProvider<CTX, Output = InterpreterResult>,
{
    ...
    fn inspect_one_tx(&mut self, tx: Self::Tx) -> Result<Self::ExecutionResult, Self::Error> {
        ...
        KasplexEvmHandler::<_, _, EthFrame>::new().inspect_run(&mut self.inner)
    }
}

// Inspect the commit EVM, here we use the same implementation as `RevmEvm`.
impl<CTX, INSP, P> InspectCommitEvm for KasplexEvm<CTX, INSP, P>
where
